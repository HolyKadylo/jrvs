#!/usr/bin/env python3
"""layerAssigner: the pipeline hub.
...
token -- media, timestamp, and word tokens alike -- and hands the structured line
to writerToDatabase, which appends it to chatLog and accumulates inline weights.

Input weights are directed by inputThinker. Output weights default to 1 (a hook is
left for a symmetric outputThinker-directed policy).
"""
import re
import threading
...
_lock = threading.Lock()  # chatLog has a single writer process; serialize records
